...
export class DatabaseStorage implements IStorage {
    ...
    try {
      ...
      if (existingIndex >= 0) {
        ...
        const newSession: VideoAnalyticsSession = {
          ...
          isAdminSession: session.isAdminSession || false,
          totalSessions: 1,
          createdAt: new Date(),
          lastActivity: new Date(),
        };
  ...
  async createOrUpdateAnalyticsSession(session: InsertVideoAnalyticsSession): Promise<VideoAnalyticsSession> {
    ...
    try {
      ...
      const existing = await database.select()
        .from(videoAnalyticsSessions)
        .where(eq(videoAnalyticsSessions.sessionId, session.sessionId))
        .limit(1);

      if (existing.length > 0) {
        // Update existing session
        const updated = await database.update(videoAnalyticsSessions)
          .set({
            lastActivity: new Date(),
        ...
        // Create new session
        const result = await database.insert(videoAnalyticsSessions).values(session).returning();
        return result[0];
      }
    } catch (error) {
      console.error("Error creating/updating analytics session:", error);
      throw error;
    }
  }
  ...
  async getAnalyticsSession(sessionId: string): Promise<VideoAnalyticsSession | undefined> {
    ...
    try {
      const result = await database.select()
        .from(videoAnalyticsSessions)
        .where(eq(videoAnalyticsSessions.sessionId, sessionId))
        .limit(1);
      return result[0];
    } catch (error) {
      console.error("Error fetching analytics session:", error);
      return undefined;
    }
  }
  ...
  async recordVideoView(view: InsertVideoAnalyticsView): Promise<VideoAnalyticsView> {
    ...
    try {
      const result = await database.insert(videoAnalyticsViews).values(view).returning();
      return result[0];
    } catch (error) {
  ...
  async updateVideoView(id: string, view: Partial<InsertVideoAnalyticsView>): Promise<VideoAnalyticsView | undefined> {
    ...
    try {
      const result = await database.update(videoAnalyticsViews)
        .set({ ...view, lastWatchedAt: new Date() })
        .where(eq(videoAnalyticsViews.id, id))
        .returning();
      return result[0];
    } catch (error) {
  ...
  async updateAnalyticsSettings(settings: Partial<InsertVideoAnalyticsSettings>): Promise<VideoAnalyticsSettings> {
    const database = getDb();
    if (!database) {
      throw new Error("Database unavailable for analytics");
    }
  ...
  async resetAnalytics(resetType: 'all' | 'views' | 'sessions'): Promise<boolean> {
    ...
    try {
      if (resetType === 'all' || resetType === 'views') {
        await database.delete(videoAnalyticsViews);
        await database.delete(videoAnalyticsStats);
      }
      if (resetType === 'all' || resetType === 'sessions') {
        await database.delete(videoAnalyticsSessions);
      }
  ...
  private async getAnalyticsSessionsFromFile(filters: {
    ...
    try {
      ...
      if (filters.excludeAdmin) {
        filteredSessions = filteredSessions.filter(s => !s.isAdminSession);
      }
