...
# Use the DATABASE_PASSWORD from environment (Replit secret)
if [ -z "$DATABASE_PASSWORD" ]; then
    echo "ERROR: DATABASE_PASSWORD not found in environment"
    echo "This should be set automatically from Replit secrets"
    exit 1
fi
...
echo 'Look for "DATABASE_PASSWORD validated" and "Database connected successfully" above'
